...
public class Parser {
	...
	public static String prefix_path = "/Users/"+System.getProperty("user.name")+"/research/experiments-data/public-data/performance/flash-data/Flash-MultiConfig/Data/";
	...
	public static LinkedHashMap<Integer, List<Double>> raw_map = new LinkedHashMap<Integer, List<Double>>();
	...
	public static String selected = "SS-M";
    ...
    public static void read(String name){
    	...
    	double time = 0.0;
    	try {
			BufferedReader reader = new BufferedReader(new FileReader(prefix_path+"name+".csv"));
			String line = null; 
		...
		for(int i = 0; i < list.size(); i++) {
			...
			if (subList.size() == 1) {
				...
				for(int j = 0; j < d.length; j++) {
					...
					if(!raw_map.containsKey(x)) {
						raw_map.put(x, new ArrayList<Double>());
					}
					
					raw_map.get(x).add(d[j]);
				}
	...
	public static List<String> getSeeds(){
		...
		/*if(1==1) {
			for (int i = 0; i < no; i++) {
				int k = PseudoRandom.randInt(0,seeds.size()-1);
				list.add(seeds.get(k));
			}
		}*/
